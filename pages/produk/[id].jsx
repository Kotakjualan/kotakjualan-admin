...
// import { getDetailProduk, sendNewProdukMerchant, updateProdukMerchant } from '../../api';
...
export default function UpdateProduk(){
  ...
  // const {resDetailProduk, loadingDetailProduk, notFoundDetailProduk} = getDetailProduk(query?.id)
  ...
  // useEffect(()=>{
  //   if(query?.id != null){
  //     setName(resDetailProduk?.product_title)
  //     setDesc(resDetailProduk?.product_description)
  //     setPrice(resDetailProduk?.product_price)
  //     setStok(resDetailProduk?.available_stock)
  //     setImagePreview(resDetailProduk?.product_image)
  //   }
  // },[resDetailProduk])
  ...
  const handlerClick = async () => {
    ...
    // const res = await updateProdukMerchant(query?.id, name, desc, price, image, stok)
  ...
  return(
    <Sidebar title="Tambah Produk - Halalpedia Express" description="Page ini akan memberikan kamu akses untuk menambahkan barang">      
      <div className="w-full bg-white px-4 py-7 rounded mb-10">
        <p className="text-2xl text-gray-800 font-semibold">Edit Produk</p>
        <p className="text-sm text-gray-600 mb-7">Page ini akan memberikan kamu akses untuk mengedit produk</p>
        ...
        <div className="flex justify-end">
          <button onClick={handlerClick} className="bg-green-500 mt-5 text-sm px-4 hover:bg-green-400 py-2 rounded text-white font-semibold">
            {(!isClick)?"Simpan Perubahan":"Loading.."}
          </button>
        </div>
